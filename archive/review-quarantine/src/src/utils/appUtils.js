...
export function weakAreaEstimator(history) {
  const counts = {};
...
export function withAutoStudyPlan(nextDetails, previousDetails = {}) {
  ...
  if (!merged.state && merged.location) merged.state = merged.location;
  if (!merged.location && merged.state) merged.location = merged.state;
...
export function buildSevenDayRevenueTrend(history, interstitialEcpm, rewardedEcpm) {
  ...
  for (let offset = 6; offset >= 0; offset--) {
    ...
    days.push({
      ...entry,
      revenue: calculateEstimatedRevenue(entry.interstitialShown, entry.rewardedCompleted, interstitialEcpm, rewardedEcpm),
...
export { CASE_PROGRESS_STAGES, CASE_STAGE_CHECKLISTS };